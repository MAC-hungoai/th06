            ...
            <div key={product.id} className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-4">
              <div className="h-20 w-28 overflow-hidden rounded-xl bg-slate-100">
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.currentTarget.src = "https://placehold.co/1200x800/e2e8f0/334155?text=No+Image";
                  }}
                />
              </div>
              ...
              <div className="flex flex-1 flex-col gap-2">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <Link to={`/product/${product.id}`} className="line-clamp-1 font-semibold hover:underline">
                      {product.name}
                    </Link>
                    <div className="mt-1 text-sm text-slate-600">
                      Đơn giá: <span className="font-medium">{formatVND(product.price)}</span>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-slate-600">
                    Thành tiền:{" "}
                    <span className="font-semibold text-slate-900">{formatVND(qty * product.price)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="sticky top-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-2 text-sm">
              <div className="flex justify-between text-slate-600">
                <span>Số lượng</span>
                <span className="font-medium text-slate-900">{cart.count}</span>
              </div>
              <div className="flex justify-between text-slate-600">
                <span>Tạm tính</span>
                <span className="font-medium text-slate-900">{formatVND(cart.subtotal)}</span>
              </div>
              <div className="flex justify-between text-slate-600">
                <span>Phí ship</span>
                <span className="font-medium text-slate-900">{formatVND(0)}</span>
              </div>
            ...
            <button className="mt-5 w-full rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">
              Thanh toán (demo)
            </button>

            <p className="mt-3 text-xs text-slate-500">Demo! không tích hợp cổng thanh toán.</p>
          </div>
        </div>
      </div>
    </div>
